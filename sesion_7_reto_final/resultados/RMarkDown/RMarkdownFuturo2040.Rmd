...
png("resultados/variables2040.png", width=2000, height=2000, pointsize=20)
plot(variables2040)
dev.off()
...
variables2040_tabla<-as.data.frame(variables2040)

#ELIMINA LOS VALORES NULOS
variables2040_tabla<-na.omit(variables2040_tabla)

#MATRIZ DE CORRELACIÓN
help(cor)
variables2040_correlacion<-cor(variables2040_tabla)

#MATRIZ DE DISTANCIAS ('ABS' = VALOR ABSOLUTO, PARA ELIMINAR CORRELACIONES NEGATIVAS)
help(as.dist)
help(abs)
variables2040_dist<-abs(as.dist(variables2040_correlacion))

#CLUSTER DE VARIABLES SEGÚN LA DISTANCIA (MENOR DISTANCIA = MAYOR CORRELACIÓN)
help(hclust)
variables2040_cluster<-hclust(1-variables2040_dist)

#GRAFICO DEL CLUSTER DE CORRELACIONES 
plot(variables2040_cluster)

#GRÁFICO DEL CLUSTER DE CORRELACIONES EXPORTADO A PDF
pdf("correlacion2040.pdf", width=8, height=11.5, pointsize=20)
plot(variables2040_cluster)
dev.off()

#SELECCIONAMOS LAS VARIABLES CON LAS QUE QUEREMOS TRABAJAR
variables2040_tabla2<-data.frame(variables2040_tabla$PA,variables2040_tabla$topo_posic,variables2040_tabla$sol_rad_sum,variables2040_tabla$huella_humana,variables2040_tabla$PV,variables2040_tabla$topo_pend)

names(variables2040_tabla2)<-c("PA","topo_posic","sol_rad_sum","huella_humana","PV","topo_pend")
...
p<-predict(variables2040, m2)
plot(p)


# Generar mapa en formato .asc
p_raster<-raster(p)
writeRaster(p, "modelo_futuro2040.asc")
